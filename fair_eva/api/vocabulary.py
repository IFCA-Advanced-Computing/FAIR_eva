...
logger = logging.getLogger("api.vocabulary")
APP_DIRNAME = os.path.abspath(
    ...
    def collect(self, search_item=None, perform_login=False):
        ...
        if not self.enable_remote_check or error_on_request:
            ...
            if os.path.isabs(self.local_path):
                self.local_path_full = self.local_path
            else:
...
class FAIRsharingRegistry(VocabularyConnection):
    ...
    def _local_collect(self):
        with open(self.local_path_full, "r") as f:
            content = json.load(f).get("data", [])
            logger.debug(
                "Successfully loaded local FAIRsharing cache with %s records"
                % len(content)
            )
    ...
    def collect(self, search_topic):
        ...
        if not remote_path:
            ...
            separator = "&" if "?" in remote_path else "?"
            remote_path_with_query = f"{remote_path}{separator}q={search_topic}"
...
class Vocabulary:
    ...
    def get_ror(self, uri):
        ...
        return vocabulary.collect(term=uri)
