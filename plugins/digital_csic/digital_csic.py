...
class Digital_CSIC(Evaluator):
    ...
    def __init__(self, item_id, oai_base=None, lang="en"):
        ...
        try:
            ...
            self.terms_relations_only_data = ast.literal_eval(
                config[plugin]["terms_relations_only_data"]
            )
        ...
        try:
            ...
            for _ in range(MAX_RETRIES):
                ...
                if r.status_code == 200:
                    ...
                    insert = (
                        "INSERT INTO debug (status_code, request, message, comment) VALUES(%i, '%s', '%s', '%s')"
                        % (r.status_code, url, r.text[0:50], item_pid)
                    )
                    c = conn.cursor()
                    c.execute(insert)
                    conn.commit()
            ...
            if r.status_code == 200:
                ...
                for _ in range(MAX_RETRIES):
                    ...
                    if r.status_code == 200:
                        ...
                        insert = (
                            "INSERT INTO debug (status_code, request, message, comment) VALUES(%i, '%s', '%s', '%s')"
                            % (r.status_code, url, r.text[0:50], "Metadata access")
                        )
                        c = conn.cursor()
                        c.execute(insert)
                        conn.commit()
            ...
            for _ in range(MAX_RETRIES):
                ...
                if r.status_code == 200:
                    ...
                    insert = (
                        "INSERT INTO debug (status_code, request, message, comment) VALUES(%i, '%s', '%s', '%s')"
                        % (r.status_code, url, r.text[0:50], "File list access")
                    )
                    c = conn.cursor()
                    c.execute(insert)
                    conn.commit()
            ...
            insert = (
                "INSERT INTO debug (status_code, request, message, comment) VALUES(%i, '%s', '%s', '%s')"
                % (r.status_code, url, r.text[0:50], "File list retrieval")
            )
            c = conn.cursor()
            c.execute(insert)
            conn.commit()
    ...
    def rda_i3_03m(self):
        ...
        try:
            ...
            logging.error("Error in I3_03M: %s" % e)
        return (points, msg)
    ...
    def rda_i3_03m(self):
        ...
        md_term_list = pd.DataFrame(
            self.terms_relations_only_data, columns=["term", "qualifier"]
        )
        md_term_list["text_value"] = [""] * len(md_term_list)
        md_term_list = ut.check_metadata_terms(self.metadata, md_term_list)
        try:
